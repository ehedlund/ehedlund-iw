...
def extract_policy_links_from_warc(warc_path):
    # try:
    ...
    warc_id = 1
    # warc_id = warc_path.rsplit("-", 1)[-1].split(".")[0]
    record_cnt = 0
    page_cnt = 0
    t0 = time()
    ...
    domain_links = {}
    out_csv_path = join(dirname(dirname(warc_path)), "warc_csv_out", "%s.csv"
                        % basename(warc_path))
...
def extract_policy_links_from_warc_files(directory):
	#todo: iterate through directory and call extract_policy_links_from_warc on each
    

    # make list of all warc files
    warc_urls = list()
    for warc_file in os.listdir(directory):
        warc_path = os.path.join(directory, warc_file)
        warc_urls.append(warc_path)
...
if __name__ == '__main__':
    t0 = time()
    extract_policy_links_from_warc_files("warc")
